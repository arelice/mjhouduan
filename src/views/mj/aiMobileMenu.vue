...
import OpenAISettings from './OpenAISettings.vue'; // 确保路径正确
import { NDrawerContent, NDrawer } from "naive-ui";
import { isDisableMenu } from '@/api';

const st = ref({ show: true });
const showSettings = ref(false); // 控制 OpenAISettings 组件的显示
...
<template>
  <div class="bg-gray-100 dark:bg-[#282832] h-[55px] flex justify-around items-center dark:text-white/70">
    ...
    <div class="flex items-center justify-center flex-col" @click="toggleSettings">
      <SvgIcon icon="ri:settings-line" class="text-3xl"></SvgIcon>
      <div class="text-[13px]">设置</div>
    </div>
  </div>
  ...
  <n-drawer v-model:show="showSettings" class="!h-[90vh] !max-h-[660px]" placement="right">
    <n-drawer-content style="--n-body-padding:0" class="h-full">
      <OpenAISettings @close="toggleSettings" />
    </n-drawer-content>
  </n-drawer>
</template>
